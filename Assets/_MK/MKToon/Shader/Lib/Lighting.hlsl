...
//////////////////////////////////////////////////////
	...
	inline half LightingDirectAdditional(inout Surface surface, in MKSurfaceData surfaceData, in MKPBSData pbsData, in MKLight light, in MKLightData lightData, out half4 finalLightColor)
	{
			...
			#ifdef MK_LEGACY_RP
				#ifdef USING_DIRECTIONAL_LIGHT
					diffuse.a *= lerp(light.shadowAttenuation, 1, UnityComputeShadowFade(UnityComputeShadowFadeDistance(surfaceData.positionWorld, dot(CAMERA_POSITION_WORLD - surfaceData.positionWorld, MATRIX_V[2].xyz))));
				#else
					diffuse.a *= light.attenuation;
				#endif
			#else
				diffuse.a *= light.shadowAttenuation;
			#endif
